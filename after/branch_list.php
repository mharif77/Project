		...
		<div class="card-body">
			<table class="table tabe-hover table-bordered" id="list">
				...
				<tbody>
					<?php
					$i = 1;
					$qry = $conn->query("SELECT * FROM branches order by Thana asc, Divison asc ");
					while($row= $qry->fetch_assoc()):
					?>
					<tr>
						<td class="text-center"><?php echo $i++ ?></td>
						<td class=""><b><?php echo $row['country'] ?></b></td>
						<td class=""><b><?php echo $row['branch_code'] ?></b></td>
						<td><b><?php echo ucwords($row['Divison']) ?></b></td>
						<td><b><?php echo ucwords($row['zip_code']) ?></b></td>
						<td><b><?php echo ucwords($row['Thana']) ?></b></td>
